...
export class Repo<T extends AggregateRoot<K>, D extends Entity<K>, K> {
  ...
  public async replace(id: K, item: T): Promise<boolean> {
    ...
    const ritem = this._mapper.mapToData(item);
    return this._store.replace(id, ritem);
  }

  public replaceSync(id: K, item: T): void {
    ...
    const ritem = this._mapper.mapToData(item);
    return this._store.replaceSync(id, ritem);
  }
}
